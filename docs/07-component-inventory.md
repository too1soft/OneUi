...
跨平台更新：组件表的主线成熟度以 Windows 为基线。Linux X11/Wayland 已在 WSLg 构建运行，
Cocoa 已接入源码但未构建；运行时服务和原生验收状态见 [后端矩阵](37-native-desktop-backends.md)。
不要把下面的 C++/C/Rust API 覆盖列解读为每个操作系统都已验收。
