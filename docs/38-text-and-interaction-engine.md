...
缓存按硬段落复用，键包括文本、字体、语言、方向、宽度、行高、缩放、截断选项和富文本 span。
只读 `Label` 可将非重叠的行内字号、字重、斜体、等宽字体、装饰线及前景/背景色提交给同一
SkParagraph；前景色在绘制阶段解析，保留布局缓存和主题颜色更新语义。普通 `setText` 与文本绑定
会清除旧 span，避免异步文本更新继承过期样式。
...
以下数量、产物体积和日志仅描述该日期的构建。后续 Slider、Tabs 原位编辑、工作区组合、
Win7 和渲染修改必须使用对应提交重新验证，不能沿用此表宣称当前全量通过。
...
## 终端格宽与局部重绘回归（2026-09-15）

VT 终端使用 `drawTextCells`，不能用 SkParagraph 的字体回退结果计算网格。
曾出现缺失字体或比例字体请求被段落引擎回退为 UI 字体，而格子绘制器另选等宽字体的情况：
完整行与局部重绘使用不同起点，造成文字重复、错位和光标漂移。

`Canvas::measureTextCellsWidth` 与 `drawTextCells` 共享实际字体选择和 advance；
`TerminalView` 仅用该方法计算格宽。普通 Label/TextField 等段落排版路径不变。
自定义 C++ Canvas 后端如覆写格子绘制的字体策略，也应覆写对应测量；
C++ 消费者需与头文件一起重编译，C ABI 及 Rust 调用方式不变。

`oneui_terminal_raster_tests` 使用真实 Skia 栅格图，比较相同网格完整绘制与剪裁重绘结果。
覆盖默认、有效、缺失和比例字体请求，ASCII、加粗、颜色切换、中文宽格、组合音标，
以及 100%/125%/150% 缩放。每通道差异大于 8 才计入变化像素，单场景允许至多 8 个
浮点抗锯齿舍入像素；修复前缺失/默认字体有数千变化像素，修复后当前 Windows 上
100% 为 0，高 DPI 为 2–6。该测试不依赖远端服务器，也不会记录真实终端内容。

### Label 实际文字尺寸审计

布局快照的 Label 节点增加 `naturalTextSize`、`paintFontSize`、
`paintFontWeight` 和 `textAlign`。自然尺寸沿用该 Label 的字体族、字号、字重、
富文本跨度及 DPI，但关闭换行和省略，以判断文字是否真的放得下；仅比较控件
frame 的包含关系不能发现内容省略。该测量只在请求布局快照时执行，不增加空闲重绘。
对于允许换行或有意省略的标签，应使用相应策略验收，不能统一要求自然宽度不超过控件。
